...
const Footer = () => {
  ...
  const footerLinks = [
    ...
    },
    {
      title: 'Contact',
      links: [
        { name: '881 Idolofiya drive, Mount Moriah, Durban, 4051', path: '#contact', section: 'contact' },
        { name: 'projects@dhreno.co.za', path: 'mailto:projects@dhreno.co.za' },
        { name: '084 959 3786', path: 'tel:+27849593786' },
        { name: 'WhatsApp Us', path: 'https://wa.me/27849593786' },
      ],
    },
  ];
      ...
      <div className="container-custom">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          >
            <a 
              ...
              aria-label="Renovations - Back to top"
            >
              <img 
                ...
                {column.links.map((link, linkIndex) => (
                  <li key={link.name}>
                    {link.path.startsWith('#') ? (
                      <a
                        href={link.path}
                        className="text-gray-400 hover:text-primary-500 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-gray-900 rounded-sm px-1"
                        onClick={(e) => link.section ? scrollToSection(link.section, e) : e.preventDefault()}
                      >
                        {link.name}
                      </a>
                    ) : (
        ...
        <div className="border-t border-gray-800 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center">
          <p className="text-gray-500 text-sm mb-4 md:mb-0">
            © {currentYear} DH INTERIOR AND RENOVATIONS (pty) ltd
          </p>
